...
frame_file.pack(fill="x", padx=5, pady=5, ipady=5) # padx=5, pady=5 간격띄우기

btn_add_file = Button(frame_file, padx=5, pady=5, width=12, text="파일추가")
btn_add_file.pack(side="left")

btn_del_file = Button(frame_file, padx=5, pady=5, width=12, text="선택삭제")
...
txt_dest_path.pack(side="left", fill="x", expand=True, padx=5, pady=5, ipady=5 ) # ipady 엔트리 높이 조정
...
# 2. 간격 옵션
# 간격옵션 레이블
lbl_space = Label(frame_option, text="가로넓이", width=10)
...
# 3. 파일포맷 옵션
# 파일포맷 옵션 레이블
lbl_format = Label(frame_option, text="포맷", width=10)
...
p_var = DoubleVar()
progress_bar = ttk.Progressbar(frame_progress, maximum=100, length=150, variable=p_var)
...
root.resizable(False, False) # x(너비), y(높이) 창크기 고정
